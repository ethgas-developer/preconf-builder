...
impl L1Config {
    ...
    pub fn get_relay_secret_key(&self) -> eyre::Result<String> {
        if self.optimistic_enabled || self.relay_secret_key.is_none() {
            return self.optimistic_relay_secret_key.clone().value();
        }
        self.relay_secret_key.clone().unwrap().value()
    }
    ...
    fn default() -> Self {
        Self {
            ...
            builders: vec![
                ...
                BuilderConfig {
                    name: "preconf-ordering".to_string(),
                    builder: SpecificBuilderConfig::OrderingBuilder(OrderingBuilderConfig {
                        discard_txs: false,
