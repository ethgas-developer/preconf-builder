...
pub struct SubmissionConfig {
    ...
    pub min_time_between_block_check_ms: Duration,
}
...
/// Every min_time_between_block_check_ms (default: 5ms) It will take a new best block produced by builders and submit it.
///
...
async fn run_submit_to_relays_job(
    ...
    'submit: loop {
        ...
        if time_since_submit < config.min_time_between_block_check_ms {
            sleep(config.min_time_between_block_check_ms - time_since_submit).await;
        }
        ...
        if config.dry_run {
                ...
                .await
            {
                ...
                Err(ValidationError::UnableToValidate(err)) => {
